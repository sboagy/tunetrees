import { sqliteTable, AnySQLiteColumn, uniqueIndex, index, integer, text, foreignKey, primaryKey, real } from "drizzle-orm/sqlite-core"
...
(table) => [
	uniqueIndex("daily_practice_queue_user_ref_playlist_ref_window_start_utc_tune_ref_unique").on(table.userRef, table.playlistRef, table.windowStartUtc, table.tuneRef),
...
export const genreTuneType = sqliteTable("genre_tune_type", {
	...
	tuneTypeId: text("tune_type_id").notNull().references(() => tuneType.id),
},
(table) => [
...
(table) => [
	uniqueIndex("instrument_private_to_user_instrument_unique").on(table.privateToUser, table.instrument),
...
export const note = sqliteTable("note", {
	...
	tuneRef: integer("tune_ref").notNull().references(() => tune.id),
...
export const playlist = sqliteTable("playlist", {
	...
	userRef: integer("user_ref").notNull().references(() => userProfile.id),
	name: text(),
...
(table) => [
	uniqueIndex("playlist_user_ref_instrument_ref_unique").on(table.userRef, table.instrumentRef),
]);

export const playlistTune = sqliteTable("playlist_tune", {
	playlistRef: integer("playlist_ref").notNull().references(() => playlist.playlistId),
	tuneRef: integer("tune_ref").notNull().references(() => tune.id),
	current: text(),
	learned: text(),
	scheduled: text(),
...
(table) => [
	uniqueIndex("practice_record_tune_ref_playlist_ref_practiced_unique").on(table.tuneRef, table.playlistRef, table.practiced),
...
export const prefsSchedulingOptions = sqliteTable("prefs_scheduling_options", {
	userId: integer("user_id").primaryKey().notNull().references(() => userProfile.id),
...
export const tableState = sqliteTable("table_state", {
	...
	playlistId: integer("playlist_id").notNull().references(() => playlist.playlistId),
	settings: text(),
...
export const tableTransientData = sqliteTable("table_transient_data", {
	userId: integer("user_id").notNull().references(() => userProfile.id),
	tuneId: integer("tune_id").notNull().references(() => tune.id),
	playlistId: integer("playlist_id").notNull().references(() => playlist.playlistId),
	purpose: text(),
...
(table) => [
	primaryKey({ columns: [table.userId, table.tuneId, table.playlistId], name: "table_transient_data_user_id_tune_id_playlist_id_pk"})
]);
...
(table) => [
	uniqueIndex("tag_user_ref_tune_ref_tag_text_unique").on(table.userRef, table.tuneRef, table.tagText),
