import { pgTable, foreignKey, pgPolicy, check, serial, integer, text, timestamp, index, unique, uuid, boolean, real, primaryKey, pgView } from "drizzle-orm/pg-core"
...
export const tabGroupMainState = pgTable("tab_group_main_state", {
	...
	pgPolicy("Users can view own tab group state", { as: "permissive", for: "select", to: ["public"], using: sql`(user_id IN ( SELECT user_profile.id
   FROM user_profile
  WHERE (user_profile.supabase_user_id = auth.uid())))` }),
	pgPolicy("Users can insert own tab group state", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can update own tab group state", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can delete own tab group state", { as: "permissive", for: "delete", to: ["public"] }),
	check("check_name", sql`which_tab = ANY (ARRAY['scheduled'::text, 'repertoire'::text, 'catalog'::text, 'analysis'::text])`),
]);
...
export const tuneType = pgTable("tune_type", {
	...
	pgPolicy("Authenticated users can view tune types", { as: "permissive", for: "select", to: ["authenticated"], using: sql`true` }),
]);
...
export const tag = pgTable("tag", {
	...
	index("idx_tag_user_ref_tag_text").using("btree", table.userRef.asc().nullsLast().op("int4_ops"), table.tagText.asc().nullsLast().op("text_ops")),
	index("idx_tag_user_ref_tune_ref").using("btree", table.userRef.asc().nullsLast().op("int4_ops"), table.tuneRef.asc().nullsLast().op("int4_ops")),
	foreignKey({
	...
	pgPolicy("Users can view own tags", { as: "permissive", for: "select", to: ["public"], using: sql`(user_ref IN ( SELECT user_profile.id
   FROM user_profile
  WHERE (user_profile.supabase_user_id = auth.uid())))` }),
	pgPolicy("Users can insert own tags", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can update own tags", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can delete own tags", { as: "permissive", for: "delete", to: ["public"] }),
]);
...
export const userProfile = pgTable("user_profile", {
	...
	pgPolicy("Users can view own profile", { as: "permissive", for: "select", to: ["public"], using: sql`(auth.uid() = supabase_user_id)` }),
...
export const practiceRecord = pgTable("practice_record", {
	...
	index("idx_practice_record_id").using("btree", table.id.desc().nullsLast().op("int4_ops")),
	index("idx_practice_record_practiced").using("btree", table.practiced.desc().nullsLast().op("timestamp_ops")),
	index("idx_practice_record_tune_playlist_practiced").using("btree", table.tuneRef.asc().nullsLast().op("int4_ops"), table.playlistRef.asc().nullsLast().op("timestamp_ops"), table.practiced.desc().nullsLast().op("timestamp_ops")),
	foreignKey({
	...
	pgPolicy("Users can view own practice records", { as: "permissive", for: "select", to: ["public"], using: sql`(playlist_ref IN ( SELECT playlist.playlist_id
   FROM playlist
  WHERE (playlist.user_ref IN ( SELECT user_profile.id
           FROM user_profile
          WHERE (user_profile.supabase_user_id = auth.uid())))))` }),
	pgPolicy("Users can insert own practice records", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can update own practice records", { as: "permissive", for: "update", to: ["public"] }),
]);
...
export const tune = pgTable("tune", {
	...
	pgPolicy("Users can view public or own private tunes", { as: "permissive", for: "select", to: ["public"], using: sql`((private_for IS NULL) OR (private_for IN ( SELECT user_profile.id
   FROM user_profile
  WHERE (user_profile.supabase_user_id = auth.uid()))))` }),
	pgPolicy("Users can insert own private tunes", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can update own private tunes", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can delete own private tunes", { as: "permissive", for: "delete", to: ["public"] }),
]);
...
export const tuneOverride = pgTable("tune_override", {
	...
	pgPolicy("Users can view own tune overrides", { as: "permissive", for: "select", to: ["public"], using: sql`(user_ref IN ( SELECT user_profile.id
   FROM user_profile
  WHERE (user_profile.supabase_user_id = auth.uid())))` }),
	pgPolicy("Users can insert own tune overrides", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can update own tune overrides", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can delete own tune overrides", { as: "permissive", for: "delete", to: ["public"] }),
]);
...
export const instrument = pgTable("instrument", {
	...
	index("idx_instrument_instrument").using("btree", table.instrument.asc().nullsLast().op("text_ops")),
	index("idx_instrument_private_to_user").using("btree", table.privateToUser.asc().nullsLast().op("int4_ops")),
	foreignKey({
	...
	pgPolicy("Users can view public or own private instruments", { as: "permissive", for: "select", to: ["public"], using: sql`((private_to_user IS NULL) OR (private_to_user IN ( SELECT user_profile.id
   FROM user_profile
  WHERE (user_profile.supabase_user_id = auth.uid()))))` }),
	pgPolicy("Users can insert own private instruments", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can update own private instruments", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can delete own private instruments", { as: "permissive", for: "delete", to: ["public"] }),
]);
...
export const dailyPracticeQueue = pgTable("daily_practice_queue", {
	...
	snapshotCoalescedTs: timestamp("snapshot_coalesced_ts", { mode: 'string' }).notNull(),
	...
	index("idx_queue_generated_at").using("btree", table.generatedAt.asc().nullsLast().op("timestamp_ops")),
	index("idx_queue_user_playlist_active").using("btree", table.userRef.asc().nullsLast().op("int4_ops"), table.playlistRef.asc().nullsLast().op("bool_ops"), table.active.asc().nullsLast().op("bool_ops")),
	index("idx_queue_user_playlist_bucket").using("btree", table.userRef.asc().nullsLast().op("int4_ops"), table.playlistRef.asc().nullsLast().op("int4_ops"), table.bucket.asc().nullsLast().op("int4_ops")),
	index("idx_queue_user_playlist_window").using("btree", table.userRef.asc().nullsLast().op("int4_ops"), table.playlistRef.asc().nullsLast().op("timestamp_ops"), table.windowStartUtc.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.playlistRef],
			foreignColumns: [playlist.playlistId],
	...
	pgPolicy("Users can view own practice queue", { as: "permissive", for: "select", to: ["public"], using: sql`(user_ref IN ( SELECT user_profile.id
   FROM user_profile
  WHERE (user_profile.supabase_user_id = auth.uid())))` }),
	pgPolicy("Users can insert own practice queue items", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can update own practice queue items", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can delete own practice queue items", { as: "permissive", for: "delete", to: ["public"] }),
]);
...
export const genre = pgTable("genre", {
	...
	pgPolicy("Authenticated users can view genres", { as: "permissive", for: "select", to: ["authenticated"], using: sql`true` }),
]);
...
export const note = pgTable("note", {
	...
	index("idx_note_tune_playlist").using("btree", table.tuneRef.asc().nullsLast().op("int4_ops"), table.playlistRef.asc().nullsLast().op("int4_ops")),
	index("idx_note_tune_playlist_user_public").using("btree", table.tuneRef.asc().nullsLast().op("int4_ops"), table.playlistRef.asc().nullsLast().op("bool_ops"), table.userRef.asc().nullsLast().op("int4_ops"), table.public.asc().nullsLast().op("bool_ops")),
	index("idx_note_tune_user").using("btree", table.tuneRef.asc().nullsLast().op("int4_ops"), table.userRef.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.playlistRef],
			foreignColumns: [playlist.playlistId],
	...
	pgPolicy("Users can view own or public notes", { as: "permissive", for: "select", to: ["public"], using: sql`((user_ref IN ( SELECT user_profile.id
   FROM user_profile
  WHERE (user_profile.supabase_user_id = auth.uid()))) OR (public = true))` }),
	pgPolicy("Users can insert own notes", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can update own notes", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can delete own notes", { as: "permissive", for: "delete", to: ["public"] }),
...
export const playlist = pgTable("playlist", {
	...
	pgPolicy("Users can view own playlists", { as: "permissive", for: "select", to: ["public"], using: sql`(user_ref IN ( SELECT user_profile.id
   FROM user_profile
  WHERE (user_profile.supabase_user_id = auth.uid())))` }),
	pgPolicy("Users can insert own playlists", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can update own playlists", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can delete own playlists", { as: "permissive", for: "delete", to: ["public"] }),
]);
...
export const prefsSchedulingOptions = pgTable("prefs_scheduling_options", {
	...
	pgPolicy("Users can view own scheduling preferences", { as: "permissive", for: "select", to: ["public"], using: sql`(user_id IN ( SELECT user_profile.id
   FROM user_profile
  WHERE (user_profile.supabase_user_id = auth.uid())))` }),
	pgPolicy("Users can insert own scheduling preferences", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can update own scheduling preferences", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can delete own scheduling preferences", { as: "permissive", for: "delete", to: ["public"] }),
]);
...
export const reference = pgTable("reference", {
	...
	index("idx_reference_tune_public").using("btree", table.tuneRef.asc().nullsLast().op("int4_ops"), table.public.asc().nullsLast().op("int4_ops")),
	index("idx_reference_tune_user_ref").using("btree", table.tuneRef.asc().nullsLast().op("int4_ops"), table.userRef.asc().nullsLast().op("int4_ops")),
	index("idx_reference_user_tune_public").using("btree", table.userRef.asc().nullsLast().op("int4_ops"), table.tuneRef.asc().nullsLast().op("int4_ops"), table.public.asc().nullsLast().op("bool_ops")),
	foreignKey({
			columns: [table.tuneRef],
			foreignColumns: [tune.id],
	...
	pgPolicy("Users can view own or public references", { as: "permissive", for: "select", to: ["public"], using: sql`((user_ref IN ( SELECT user_profile.id
   FROM user_profile
  WHERE (user_profile.supabase_user_id = auth.uid()))) OR (public = true))` }),
	pgPolicy("Users can insert own references", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can update own references", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can delete own references", { as: "permissive", for: "delete", to: ["public"] }),
	check("check_favorite", sql`favorite = ANY (ARRAY[true, false])`),
	check("check_public", sql`public = ANY (ARRAY[true, false])`),
	check("check_ref_type", sql`ref_type = ANY (ARRAY['website'::text, 'audio'::text, 'video'::text])`),
]);
...
export const genreTuneType = pgTable("genre_tune_type", {
	...
	pgPolicy("Authenticated users can view genre-tune type relationships", { as: "permissive", for: "select", to: ["authenticated"], using: sql`true` }),
]);
...
export const tableState = pgTable("table_state", {
	...
	pgPolicy("Users can view own table state", { as: "permissive", for: "select", to: ["public"], using: sql`(user_id IN ( SELECT user_profile.id
   FROM user_profile
  WHERE (user_profile.supabase_user_id = auth.uid())))` }),
	pgPolicy("Users can insert own table state", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can update own table state", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can delete own table state", { as: "permissive", for: "delete", to: ["public"] }),
	check("purpose_check", sql`purpose = ANY (ARRAY['practice'::text, 'repertoire'::text, 'catalog'::text, 'analysis'::text])`),
	check("screen_size_check", sql`screen_size = ANY (ARRAY['small'::text, 'full'::text])`),
]);
...
export const playlistTune = pgTable("playlist_tune", {
	...
	pgPolicy("Users can view own playlist tunes", { as: "permissive", for: "select", to: ["public"], using: sql`(playlist_ref IN ( SELECT playlist.playlist_id
   FROM playlist
  WHERE (playlist.user_ref IN ( SELECT user_profile.id
           FROM user_profile
          WHERE (user_profile.supabase_user_id = auth.uid())))))` }),
	pgPolicy("Users can insert own playlist tunes", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can update own playlist tunes", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can delete own playlist tunes", { as: "permissive", for: "delete", to: ["public"] }),
]);
...
export const prefsSpacedRepetition = pgTable("prefs_spaced_repetition", {
	...
	pgPolicy("Users can view own SR preferences", { as: "permissive", for: "select", to: ["public"], using: sql`(user_id IN ( SELECT user_profile.id
   FROM user_profile
  WHERE (user_profile.supabase_user_id = auth.uid())))` }),
	pgPolicy("Users can insert own SR preferences", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can update own SR preferences", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can delete own SR preferences", { as: "permissive", for: "delete", to: ["public"] }),
...
export const tableTransientData = pgTable("table_transient_data", {
	...
	pgPolicy("Users can view own transient data", { as: "permissive", for: "select", to: ["public"], using: sql`(user_id IN ( SELECT user_profile.id
   FROM user_profile
  WHERE (user_profile.supabase_user_id = auth.uid())))` }),
	pgPolicy("Users can insert own transient data", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can update own transient data", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can delete own transient data", { as: "permissive", for: "delete", to: ["public"] }),
]);
export const viewPlaylistJoined = pgView("view_playlist_joined", {	playlistId: integer("playlist_id"),
	userRef: integer("user_ref"),
...
}).as(sql`SELECT p.playlist_id, p.user_ref, p.deleted AS playlist_deleted, p.instrument_ref, i.private_to_user, i.instrument, i.description, i.genre_default, i.deleted AS instrument_deleted FROM playlist p JOIN instrument i ON p.instrument_ref = i.id`);

export const practiceListJoined = pgView("practice_list_joined", {	id: integer(),
	title: text(),
	type: text(),
	structure: text(),
	mode: text(),
...
export const practiceListStaged = pgView("practice_list_staged", {	id: integer(),
	...
	latestRepetitions: integer("latest_repetitions"),
	latestDue: timestamp("latest_due", { mode: 'string' }),
	latestBackupPracticed: timestamp("latest_backup_practiced", { mode: 'string' }),
	latestGoal: text("latest_goal"),
	latestTechnique: text("latest_technique"),
	tags: text(),
	purpose: text(),
