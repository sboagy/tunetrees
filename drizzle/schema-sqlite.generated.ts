...
, (t) => [
  uniqueIndex("daily_practice_queue_user_ref_playlist_ref_window_start_utc_key").on(t.userRef, t.playlistRef, t.windowStartUtc, t.tuneRef),
...
, (t) => [
  uniqueIndex("instrument_private_to_user_instrument_key").on(t.privateToUser, t.instrument),
...
, (t) => [
  uniqueIndex("practice_record_tune_ref_playlist_ref_practiced_key").on(t.tuneRef, t.playlistRef, t.practiced),
...
export const prefsSchedulingOptions = sqliteTable("prefs_scheduling_options", {
  userId: text("user_id").notNull().references(() => userProfile.id).primaryKey(),
  acceptableDelinquencyWindow: integer("acceptable_delinquency_window").notNull().default(21),
...
, (t) => [
  index("idx_sync_change_log_changed_at").on(t.changedAt),
]
);
...
export const tableState = sqliteTable("table_state", {
  ...
  playlistId: text("playlist_id").notNull().references(() => playlist.playlistId),
...
export const tableTransientData = sqliteTable("table_transient_data", {
  userId: text("user_id").notNull().references(() => userProfile.id),
  tuneId: text("tune_id").notNull().references(() => tune.id),
  playlistId: text("playlist_id").notNull().references(() => playlist.playlistId),
...
, (t) => [
  primaryKey({ columns: [t.tuneId, t.userId, t.playlistId] }),
]
);
...
, (t) => [
  uniqueIndex("tag_user_ref_tune_ref_tag_text_key").on(t.userRef, t.tuneRef, t.tagText),
...
, (t) => [
  uniqueIndex("user_profile_supabase_user_id_key").on(t.supabaseUserId),
]
);
