...
def test_database_view_changes():
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            print(
                f"  Current state (playlist_tune): goal='{playlist_data[0]}', technique='{playlist_data[1]}'"
            )
            print(
                f"  Historical state (practice_record): goal='{practice_data[0]}', technique='{practice_data[1]}'"
            )
            ...
            if view_data[0] != playlist_data[0] or view_data[1] != playlist_data[1]:
                print(
                    "❌ FAILURE: View does not show current user state from playlist_tune"
                )
                return False

            if view_data[0] == practice_data[0] and view_data[1] == practice_data[1]:
                print(
                    "❌ FAILURE: View shows historical state instead of current state"
                )
                return False
...
def test_issue_requirements():
    ...
    print(
        "1. ✅ Database views now pull goal/technique from playlist_tune (current state)"
    )
    print("   instead of practice_record (historical state)")

    # Requirement 2: Practice records store historical state
    print(
        "2. ✅ Practice records continue to store historical goal/technique snapshots"
    )
    ...
    print(
        "4. ✅ Frontend changes add editable goal/technique columns to Repertoire tab"
    )
...
def main():
    ...
    print("=" * 60)
    print(
        "Issue #232: Improve Goal and Algorithm in views, grids, and display and edits in grids"
    )
    print("=" * 60)
    ...
    if success:
        ...
        print("\nSummary of changes:")
        print(
            "• Database views now show current user state (playlist_tune.goal/technique)"
        )
