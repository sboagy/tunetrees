    ...
    const waitForRef = () => {
      ...
      const restoreScroll = () => {
        ...
        try {
          ...
          // Optional diagnostics (enable with localStorage key: TT_DEBUG_PRACTICE_SCROLL = "true")
          const debugScroll = (() => {
            try {
              return (
                localStorage.getItem("TT_DEBUG_PRACTICE_SCROLL") === "true"
              );
            } catch {
              return false;
            }
          ...
          const debugScrollListener = () => {
            ...
            if (top <= 2) {
              ...
              console.warn(
                "[PRACTICE_SCROLL][DEBUG] scroll event: scrollTop<=2",
                {
                  top,
            ...
            try {
              if (typeof containerRef.scrollTo === "function") {
                ...
                (containerRef as any).scrollTo = (...args: any[]) => {
                  console.warn("[PRACTICE_SCROLL][DEBUG] scrollTo invoked", {
                    args,
          ...
          const pollReapply = () => {
            ...
            if (target > 0 && containerRef.scrollTop < target - 1) {
              ...
              reapplyCount++;
              console.log(
                `PRACTICE_SCROLL: re-applied scrollTop=${target} phase=guard-poll count=${reapplyCount} age=${
                  Date.now() - restoreAt()
                }ms`
              );
            }
            ...
            if (Date.now() < guardEndTime) {
              ...
              if (debugScroll) {
                console.warn(
                  `[PRACTICE_SCROLL][DEBUG] guard window ended. reapplyCount=${reapplyCount}`
                );
              }
            }
          };
