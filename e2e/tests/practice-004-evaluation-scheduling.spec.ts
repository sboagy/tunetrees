...
test.describe.serial("PRACTICE-004: Evaluation Scheduling", () => {
  ...
  test("should schedule 'Again' rating for tomorrow (not today)", async ({
    ...
    const scheduledCell = firstRow.getByRole("cell").nth(scheduledColumnIndex);
  ...
  test("should evaluate first tune as Good", async ({ page }) => {
    ...
    const evaluationsCount = page.getByText(/1 evaluation/i);
    await expect(evaluationsCount).toBeVisible({ timeout: 5000 });
  });
  ...
  test("should clear evaluation when selecting '(Not Set)'", async ({
    ...
    await expect(page.getByText(/1 evaluation/i)).toBeVisible({
      timeout: 5000,
    });
    ...
    await expect(firstDropdown).toContainText(/(Not Set)/i, { timeout: 5000 });

    // Evaluations count should be 0
    await expect(page.getByText(/0 evaluation/i)).toBeVisible({
      timeout: 5000,
    });
  });
});
