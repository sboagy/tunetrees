...
let ttPage: TuneTreesPage; // TuneTreesPage not available yet, use any for now
...
test.describe
  .serial("PRACTICE-004: Evaluation Scheduling", () => {
    ...
    test("should schedule 'Again' rating for next day minimum (not same day)", async ({
      ...
      // Verify the "Scheduled" column shows tomorrow (not today)
      // const scheduledCell = firstRow.getByRole("cell").nth(10);
      ...
      // Old test, start with, tests should pretty much NEVER contain conditional statements like this!
      // if (scheduledText) {
    ...
    test("should submit evaluations and filter completed tunes from grid", async ({
      ...
      if (initialCount > 1) {
        ...
        const secondDropdown = secondRow.locator(
          "[data-testid^='recall-eval-']"
        );
    ...
    test("should show submitted tunes with correct quality when 'Show Submitted' is enabled", async ({
      ...
      if (rowCount > 0) {
        ...
        const evaluationCells = grid.locator("td span.italic:has-text('Easy')");
