  ...
  test("should advance scheduling dates with repeated Easy evaluations over 10 days", async ({
    ...
    for (let day = 1; day <= MAX_DAYS; day++) {
      ...
      const queueDateDebug = await page.evaluate(() => ({
        queueDate: localStorage.getItem("TT_PRACTICE_QUEUE_DATE"),
        queueDateManual: localStorage.getItem("TT_PRACTICE_QUEUE_DATE_MANUAL"),
      }));
      console.log(
        `  [Day ${day}] queueDate=${queueDateDebug.queueDate}, manual=${queueDateDebug.queueDateManual}, currentDate=${currentDate.toISOString()}`
      );
      ...
      await expect
        .poll(
          async () => {
            ...
            if (!maybe) {
              console.log(
                `    [Day ${day}] latest practice record missing; queueDate=${queueDateDebug.queueDate}`
              );
              return -1;
            }
            const due = new Date(maybe.due);
            console.log(
              `    [Day ${day}] polled due=${due.toISOString()} (interval=${maybe.interval})`
            );
            return Math.floor(
      ...
      if (day < 10) {
        ...
        if (loginVisible) {
          ...
          await page
            ...
            .fill(getRequiredSharedTestPassword());
