-- ============================================================================
-- Supabase Migration: Stateless Sync Change Log
-- ============================================================================
--
-- This migration creates the sync_change_log table - a STATELESS server-side
-- log of changes for clients to pull from.
-- 
-- Different from the client's sync_push_queue which has status/operation/attempts.
--
-- Design Principles:
-- 1. The change log just records WHAT changed and WHEN
-- 2. Each CLIENT tracks its own lastSyncAt timestamp
-- 3. No per-client sync status stored server-side
-- 4. Garbage collection based on AGE, not sync status
--
-- Schema: (id, table_name, row_id, changed_at) - that's it!
-- ============================================================================

-- ============================================================================
-- DROP OLD TABLE AND RECREATE SIMPLIFIED VERSION
-- ============================================================================
...
-- Index for efficient queries by timestamp (the main query pattern)
CREATE INDEX idx_outbox_changed_at ON sync_change_log(changed_at);

-- Index for deduplication queries (same table+row)
CREATE INDEX idx_outbox_table_row ON sync_change_log(table_name, row_id);
...
-- ============================================================================
-- SIMPLIFIED TRIGGER FUNCTION - Single PK tables
-- Just records the change, no operation type needed
-- ============================================================================
CREATE OR REPLACE FUNCTION sync_change_log_single_pk()
RETURNS TRIGGER AS $$
DECLARE
    pk_col TEXT;
    row_id_val TEXT;
BEGIN
    ...
    -- Simple insert - just table, row, and timestamp
    INSERT INTO sync_change_log (id, table_name, row_id, changed_at)
    VALUES (gen_random_uuid()::text, TG_TABLE_NAME, row_id_val, sync_now_iso());
...
CREATE TRIGGER trg_daily_practice_queue_sync
    ...
    FOR EACH ROW EXECUTE FUNCTION sync_change_log_single_pk('id');
...
CREATE OR REPLACE FUNCTION sync_change_log_genre_tune_type()
RETURNS TRIGGER AS $$
DECLARE
    row_id_val TEXT;
BEGIN
    ...
    INSERT INTO sync_change_log (id, table_name, row_id, changed_at)
    VALUES (gen_random_uuid()::text, 'genre_tune_type', row_id_val, sync_now_iso());
...
CREATE TRIGGER trg_genre_tune_type_sync
    ...
    FOR EACH ROW EXECUTE FUNCTION sync_change_log_genre_tune_type();
...
CREATE TRIGGER trg_user_profile_sync
    ...
    FOR EACH ROW EXECUTE FUNCTION sync_change_log_single_pk('supabase_user_id');
...
CREATE TRIGGER trg_prefs_scheduling_options_sync
    ...
    FOR EACH ROW EXECUTE FUNCTION sync_change_log_single_pk('user_id');

-- prefs_spaced_repetition (composite PK: user_id, alg_type)
CREATE OR REPLACE FUNCTION sync_change_log_prefs_spaced_repetition()
RETURNS TRIGGER AS $$
DECLARE
    row_id_val TEXT;
BEGIN
    ...
    INSERT INTO sync_change_log (id, table_name, row_id, changed_at)
    VALUES (gen_random_uuid()::text, 'prefs_spaced_repetition', row_id_val, sync_now_iso());
...
CREATE TRIGGER trg_prefs_spaced_repetition_sync
    ...
    FOR EACH ROW EXECUTE FUNCTION sync_change_log_prefs_spaced_repetition();
...
CREATE TRIGGER trg_playlist_sync
    ...
    FOR EACH ROW EXECUTE FUNCTION sync_change_log_single_pk('playlist_id');

-- table_state (composite PK: user_id, screen_size, purpose, playlist_id)
CREATE OR REPLACE FUNCTION sync_change_log_table_state()
RETURNS TRIGGER AS $$
DECLARE
    row_id_val TEXT;
BEGIN
    ...
    INSERT INTO sync_change_log (id, table_name, row_id, changed_at)
    VALUES (gen_random_uuid()::text, 'table_state', row_id_val, sync_now_iso());
...
CREATE TRIGGER trg_table_state_sync
    ...
    FOR EACH ROW EXECUTE FUNCTION sync_change_log_table_state();
...
CREATE OR REPLACE FUNCTION sync_change_log_playlist_tune()
RETURNS TRIGGER AS $$
DECLARE
    row_id_val TEXT;
BEGIN
    ...
    INSERT INTO sync_change_log (id, table_name, row_id, changed_at)
    VALUES (gen_random_uuid()::text, 'playlist_tune', row_id_val, sync_now_iso());
...
CREATE TRIGGER trg_playlist_tune_sync
    ...
    FOR EACH ROW EXECUTE FUNCTION sync_change_log_playlist_tune();
...
CREATE OR REPLACE FUNCTION sync_change_log_table_transient_data()
RETURNS TRIGGER AS $$
DECLARE
    row_id_val TEXT;
BEGIN
    ...
    INSERT INTO sync_change_log (id, table_name, row_id, changed_at)
    VALUES (gen_random_uuid()::text, 'table_transient_data', row_id_val, sync_now_iso());
...
CREATE TRIGGER trg_table_transient_data_sync
    ...
    FOR EACH ROW EXECUTE FUNCTION sync_change_log_table_transient_data();
...
-- Call this periodically (via pg_cron or Supabase Edge Function)
-- Deletes entries older than 7 days
-- ============================================================================
CREATE OR REPLACE FUNCTION sync_change_log_cleanup(days_to_keep INTEGER DEFAULT 7)
RETURNS INTEGER AS $$
DECLARE
    deleted_count INTEGER;
BEGIN
    DELETE FROM sync_change_log 
    WHERE changed_at < to_char(
        now() AT TIME ZONE 'UTC' - (days_to_keep || ' days')::interval, 
