...
describe("worker media routes", () => {
  ...
  it("serves shared rhythm samples from the R2 vault", async () => {
    const vault = createVault();
    vault.objects.set("rhythm-samples/bodhran_bosone/bass.mp3", {
      body: "0123456789",
      contentType: "audio/mpeg",
    });
    ...
    const response = await handleMediaRequest(
      new Request(
        "https://worker.example.com/api/rhythm-samples/view?kit=bodhran_bosone&file=bass.mp3",
        {
          headers: {
            Range: "bytes=2-5",
          },
  ...
  it("falls back to legacy rhythm sample keys already uploaded under samples/bodhran", async () => {
    const vault = createVault();
    vault.objects.set("samples/bodhran/bass.mp3", {
      body: "legacy-bass",
      contentType: "audio/mpeg",
    });
    ...
    expect(response?.status).toBe(200);
    expect(response?.headers.get("Content-Type")).toBe("audio/mpeg");
    expect(await response?.text()).toBe("legacy-bass");
  });

  it("infers audio/mpeg for legacy mp3 rhythm samples uploaded without metadata", async () => {
    const vault = createVault();
    vault.objects.set("samples/bodhran/bass.mp3", {
      body: "legacy-bass",
