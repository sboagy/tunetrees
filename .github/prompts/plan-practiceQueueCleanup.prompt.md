...
| Phase 8 | Complete | `src/lib/services/queue-generator.ts` no longer imports `getDueTunesLegacy`, and the deprecated `DueTuneEntry`, `getDueTunes()`, and `getDueTunesLegacy()` exports were removed from `src/lib/db/queries/practice.ts`. |
...
- Complete: `npx playwright test e2e/tests/practice-005-date-rollover-banner.spec.ts e2e/tests/sync-002-queue-date-from-db-not-localstorage.spec.ts e2e/tests/sync-004-manual-localstorage-date-ignored.spec.ts --project=chromium --project='Mobile Chrome' --reporter=list`

### Summary

Completed phases: **1, 2, 3, 4, 5, 6, 7, and 8**.
