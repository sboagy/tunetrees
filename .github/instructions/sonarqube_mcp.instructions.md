...
- **IMPORTANT**: When running the MCP server, SONARQUBE_TOKEN must be set to a valid SonarQube token.  That token must come from running the `op` command with `.env.sonar.template`, for example `op run --env-file=\".env.sonar.template\" -- <command>`. If you encounter `SonarQube answered with Not authorized`, verify that the token is a USER token, not a project token.  If you don't have a valid token, stop and tell the user about the problem.
