...
const DEFAULT_SOURCE_BUCKET = "tunetrees-rhythm-assets";
const DEFAULT_TARGET_BUCKET = "tunetrees-rhythm-assets-staging";
const SERVICE = "s3";
const REGION = "auto";
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--apply") args.apply = true;
      ...
      console.log(
        "Usage: node scripts/sync-rhythm-assets-r2.mjs [--dry-run|--apply] [--prefix audio/kits/] [--limit N]"
      );
      process.exit(0);
    } else {
...
function getBuckets() {
  const sourceBucket = env(
    "RHYTHM_ASSETS_SOURCE_BUCKET",
    DEFAULT_SOURCE_BUCKET
  );
  const targetBucket = env(
    "RHYTHM_ASSETS_TARGET_BUCKET",
