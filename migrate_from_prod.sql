...
SELECT id, type, structure, title, mode, incipit, genre
FROM source_db.tune;

-- User Table Migration
INSERT OR REPLACE INTO main.user (id, hash, name, email, email_verified, image)
SELECT id, hash, name, email, email_verified, image
FROM source_db.user;

-- Playlist Table Migration
INSERT OR REPLACE INTO main.playlist (playlist_id, user_ref, instrument, description)
...
INSERT OR REPLACE INTO main.practice_record (playlist_ref, tune_ref, practiced, quality, id, easiness, interval, repetitions, review_date, backup_practiced, stability, elapsed_days, lapses, state)
SELECT playlist_ref, tune_ref, practiced, quality, id, easiness, interval, repetitions, review_date, backup_practiced, stability, elapsed_days, lapses, state
FROM source_db.practice_record;

-- User Annotation Set Table Migration
...
INSERT OR REPLACE INTO main.account (user_id, provider_account_id, provider, type, access_token, id_token, refresh_token, scope, expires_at, session_state, token_type)
SELECT user_id, provider_account_id, provider, type, access_token, id_token, refresh_token, scope, expires_at, session_state, token_type
FROM source_db.account;

-- Genre Table Migration
INSERT OR REPLACE INTO main.genre (id, name, region, description)
SELECT id, name, region, description
FROM source_db.genre;

-- Prefs Spaced Repetition Table Migration
INSERT OR REPLACE INTO main.prefs_spaced_repetition (alg_type, user_id, fsrs_weights, request_retention, maximum_interval)
SELECT alg_type, user_id, fsrs_weights, request_retention, maximum_interval
FROM source_db.prefs_spaced_repetition;

-- Session Table Migration
INSERT OR REPLACE INTO main.session (session_token, user_id, expires)
SELECT session_token, user_id, expires
FROM source_db.session;
...
INSERT OR REPLACE INTO main.table_transient_data (user_id, tune_id, playlist_id, purpose, note_private, note_public, recall_eval)
SELECT user_id, tune_id, playlist_id, purpose, note_private, note_public, recall_eval
FROM source_db.table_transient_data;

-- Tag Table Migration
INSERT OR REPLACE INTO main.tag (tag_id, user_ref, tune_ref, tag_text)
SELECT tag_id, user_ref, tune_ref, tag_text
FROM source_db.tag;

-- Verification Token Table Migration
INSERT OR REPLACE INTO main.verification_token (identifier, token, expires)
SELECT identifier, token, expires
FROM source_db.verification_token;
