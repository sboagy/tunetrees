...
const defaultSerializer = <T>(data: T): T => data;
...
export class HttpAdpaterManager<WithVerificationToken = boolean> {
  ...
  public async linkAccount(
    ...
    const parsedRes = await schemas.linkAccountSchema.parseAsync(
      serialize(res),
    );
  ...
  public async getSessionAndUser(account: AdapterArg_0<"getSessionAndUser">) {
    ...
    return await schemas.getSessionAndUserSchema.parseAsync(serialized);
  }
  ...
  public async updateUser(user: AdapterArg_0<"updateUser">) {
    ...
    try {
      ...
      const res = await ofetch(path, mergedOptions);
