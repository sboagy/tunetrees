...
test("test-signup-1", async ({ page }) => {
  ...
  const topSignInButton = ttPO.page.getByRole("button", { name: "New user" });
  ...
  if (process.env.NEXT_PUBLIC_MOCK_EMAIL_CONFIRMATION === "true") {
    ...
    const emailSubject = "Email Verification"; // Replace with the actual subject of the email
    ...
    const emailSignInLink = page.getByRole("link", { name: "Sign in" });
