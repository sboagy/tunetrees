...
export const getPracticeListScheduled = async (user_id: string, playlist_id: string) => {
    try {
        console.log('In getPracticeListScheduled: baseURL: %s', client.getUri())
        console.log('user_id: %s, playlist_id: %s', user_id, playlist_id)
        let response = await client.get(`/get_practice_list_scheduled/${user_id}/${playlist_id}`)
...
export const getRecentlyPracticed = async (user_id: string, playlist_id: string) => {
  try {
